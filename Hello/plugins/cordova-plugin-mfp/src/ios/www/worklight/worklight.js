...
WL.windowsProfileData[WL.EPField.SUPPORT_PUSH] = true;
...
WL.Logger = (function (jQuery) {
  ...
  var __setState = function (options) {
    ...
    if (__checkNativeEnvironment()) {
      _setNativeOptions({filters: state.filters, filtersFromServer: state.filtersFromServer, level: state.level, levelFromServer: state.levelFromServer, capture: state.capture, captureFromServer: state.captureFromServer, analyticsCapture: state.analyticsCapture, maxFileSize: state.maxFileSize});
		...
		if (WL.EnvProfile.isEnabled(WL.EPField.USES_CORDOVA)) {
			...
			var addDeviceIDHeader = function() {
				...
				function deviceIDFailureCallback(error) {
					WL.Logger.trace('deviceIDFailureCallback: ', error);
...
this.pinTrustedCertificatePublicKey = function(certificateFilename){
	...
	this.__invokeProcedure = function(invocationData, options, useSendInvoke) {
		...
		switch(environment) {
			...
			case WL.Env.WINDOWS8:
			case WL.Env.WINDOWS:
				if(invocationData.compressResponse){
					resourceRequest.addHeader("Accept-Encoding", "gzip");
				}
				break;
			default:
				break;
      ...
      var processResponse = function(transport) {
        ...
        if (transport.status === 409) {
          ...
          if (currentResourceRequest.scope !== null) {
            WLAuthorizationManager.__cacheScopeByResource(transport, currentResourceRequest.scope).always(function (){ resendRequest(++attempt, conflictAttemptCounter);});
          }
          ...
          }).always(function (){ resendRequest(++attempt, conflictAttemptCounter);});
        } else {
          ...
          WLAuthorizationManager.__cacheScopeByResource(transport, '').always(function (){resendRequest(++attempt, conflictAttemptCounter);});
        }
      };
...
var wl_authenticityChallengeHandler = WL.Client.createWLChallengeHandler("appAuthenticity");
...
var wl_directUpdateChallengeHandler = WL.Client.createWLChallengeHandler("directUpdate");
...
wl_directUpdateChallengeHandler.handleChallenge = function(challenge) {
	...
	if (WLJSX.Object.isUndefined(challenge.getCheckSum)
		...
		this.logger.error("Direct update challange is invalid: " + challenge);
		wl_directUpdateChallengeHandler.submitFailure();
		return;
	}
